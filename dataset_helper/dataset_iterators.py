...
class BengaluruOccupancyDatasetIterator(BengaluruDepthDatasetIterator):
	...
	def transform_occupancy_grid_to_points(self, occupancy_grid, threshold=0.5, device=device, skip=3):
		...
		def f(xi):
			...
			x,y,z = [
				...
				(j - self.occupancy_shape[1]/2) * self.grid_size[1] / (self.occupancy_shape[1]/2),
				(k - self.occupancy_shape[2]/2) * self.grid_size[2] / (self.occupancy_shape[2]/2)
			]
	...
	def transform_points_to_occupancy_grid(self, velodyine_points_orig):
		...
		for index in range(velodyine_points.shape[0]):
			...
			if np.isinf(x) or np.isinf(y) or np.isinf(z) or np.isnan(x) or np.isnan(y) or np.isnan(z):
				continue

			i, j, k = [
				int((x*self.occupancy_shape[0]//2)//self.grid_size[0])*2,
				int((y*self.occupancy_shape[1]//2)//self.grid_size[1] + self.occupancy_shape[1]//2),
				int((z*self.occupancy_shape[2]//2)//self.grid_size[2] + self.occupancy_shape[2]//2)
			]
	...
	def __getitem__(self, key):
		...
		frame['velodyine_points_camera'] = occupancy_grid_data['velodyine_points_camera']
...
class AndroidDatasetIterator:
	...
	def __init__(self, 
		...
		# print('original self.frame_count', self.frame_count)
		# # frame_number = round(self.frame_count * 0.5)
		# frame_number = 0
		...
		# print('new self.frame_count', self.frame_count)
		# exit()
		...
		csv_final_frame_number = round(self.expected_duration * self.fps / 1000.0)
		video_final_frame_number = round(self.duration * self.fps / 1000.0)

		assert csv_final_frame_number <= self.frame_count, "csv_final_frame_number > self.frame_count" + str(csv_final_frame_number) + " " + str(self.frame_count)
		...
		cached_trajectory_folder = os.path.join(self.folder_path, )
		os.makedirs(cached_trajectory_folder, exist_ok=True)
		self.cached_trajectory_path = os.path.join(cached_trajectory_folder, os.path.basename(self.csv_path).replace(".csv", "_traj.csv"))
	...
	def __getitem__(self, key):
		...
		assert frame_number < self.frame_count, "Frame number out of bounds: " + str(frame_number) + " >= " + str(self.frame_count)
	...
	def compute_slam(self, scale_factor=0.25, enable_plot=False, plot_3D_x=250, plot_3D_y=500,):
		...
		for img_id in tqdm(range(start_id, self.__len__(), 1)):
...
def main_grid(point_cloud_array, plot2D, plot3D):
	...
	for frame in depth_dataset:
		...
		grid_points = depth_dataset.transform_occupancy_grid_to_points(occupancy_grid_torch)
...
def main_traj(point_cloud_array, plot2D, plot3D):
	phone_iter = AndroidDatasetIterator(
